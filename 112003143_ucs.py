...
for i in range(65, 65+num_nodes):
    locations[chr(i)] = np.ravel(np.random.randint(1000,size=(1,2)))
...
heap = {}
cost['A'] = g_value(start_node,start_node)
heap[start_node] = cost['A']
closed_list = []
N = start_node
successor = []
goal_test = False
...
while (len(open_list)):
    ...
    for node in graph[front_node]:
        g = cost[front_node]+g_value(front_node,node)
